...
imagetyperzapi.submit_hcaptcha = function (d) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if(d.user_agent) data.useragent = d.user_agent;
        // domain
        if (d.domain) data.apiEndpoint = d.domain;
...
imagetyperzapi.submit_turnstile = function (d) {
    var ic = imagetyperz_config;    // get config obj
    return new Promise((resolve, reject) => {
        var page_url = d.page_url;
        var sitekey = d.sitekey;
        ...
        if (d.domain) data.apiEndpoint = d.domain
        if (d.action) data.taction = d.action
        if (d.cdata) data.data = d.cdata

        // make post request
        $.post(ic.endpoints.TURNSTILE_ENDPOINT, data).done(function (resp) {
            // check for error
            if (resp.indexOf('ERROR:') !== -1) return reject(resp);
            return resolve(JSON.parse(resp)[0].CaptchaId);
        }).fail(function (err) {
