...
imagetyperzapi.solve_captcha = function (captcha, optional_parameters) {
    var ic = imagetyperz_config;    // get config obj
    return new Promise((resolve, reject) => {
...
// retrieve geetest
imagetyperzapi.retrieve_geetest = function (captcha_id) {
    var ic = imagetyperz_config;    // get config obj
    return new Promise((resolve, reject) => {
        var data = {}, url = undefined;
        if (ic.username && ic.password) {
            url = ic.endpoints.GEETEST_RETRIEVE_ENDPOINT;
            // legacy auth
            data['username'] = ic.username;
            data['password'] = ic.password;
        }
        ...
        function check_progress() {
            $.post(url, data).done(function (resp) {
                ...
                if (resp.indexOf('ERROR:') !== -1) return reject(resp);
                var s = resp.split(';;;');
                if(s.length !== 3) reject('invalid geetest response: ' + resp);
                resolve({
                    'challenge': s[0],
                    'validate': s[1],
                    'seccode': s[2]
